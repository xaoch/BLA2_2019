...
```{r}
selectInput("variable", label = "Variable to show:",
            choices = var_df$variables, selected = "school")
```
...
renderPlot({
  ...
  if (variableType=="numerical"){
   hist(student_data[[input$variable]],main=paste("Distribution of",input$variable), xlab=input$variable,ylab="Students")
  }
  if (variableType=="ordinal"){
      factors<-factor(student_data[[input$variable]], order = TRUE)
      plot(factors,col = rainbow(15),main=paste("Distribution of",input$variable),xlab=input$variable,ylab="Students")
  }
  if (variableType=="nominal"){
          plot(student_data[[input$variable]],col=rainbow(15),main=paste("Distribution of",input$variable),xlab=input$variable,ylab="Students")
  }
...
```{r}
selectInput("variable1", label = "First Variable:",
            choices = var_df$variables, selected = "school")

uiOutput("secondSelection")

output$secondSelection <- renderUI({
                selectInput("variable2", label="Second Variable:", 
                choices = var_df[var_df$variable!=input$variable1,]$variables, selected=var_df[var_df$variable!=input$variable1,]$variables[0])
        })
```
...
renderPlot({
    ...
    if (type1=="numerical" & type2=="numerical"){
        plot(student_data[[input$variable1]],student_data[[input$variable2]],main=title,xlab=input$         variable1,ylab=input$variable2)
    }
    if(type1=="numerical" & (type2=="ordinal" | type2=="nominal")){
       formula<-as.formula(paste(input$variable1,"~",input$variable2)) 
       boxplot(formula,data=student_data,main=title,xlab=input$variable1,ylab=input$variable2)    
    }
    if((type1=="ordinal" | type1=="nominal") & (type2=="ordinal" | type2=="nominal")){
      formula<-as.formula(paste("~",input$variable1,"+",input$variable2))  
      xt<-xtabs(formula, data = student_data)
      plot(xt, col = rainbow(15), main=title,xlab=input$variable1,ylab=input$variable2)
    }
